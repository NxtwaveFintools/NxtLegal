...
const POSTHandler = withAuth(async (request: NextRequest, { session, params }) => {
  ...
  try {
    ...
    await contractUploadService.finalizeReplaceSupportingDocument({
    ...
    const responseData = okResponse({ success: true as const })
    ...
    logger.warn('Supporting document replacement finalize failed', {
      error: String(error),
      errorCode: isAppError(error) ? error.code : 'INTERNAL_ERROR',
    })
    const status = isAppError(error) ? error.statusCode : 500
    const code = isAppError(error) ? error.code : 'INTERNAL_ERROR'
    const message = isAppError(error) ? error.message : 'Failed to finalize supporting document replacement'
