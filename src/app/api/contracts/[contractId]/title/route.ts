...
import { contractRenameTitleSchema } from '@/core/domain/contracts/schemas'
...
const PATCHHandler = withAuth(async (request: NextRequest, { session, params }) => {
  try {
    ...
    const payload = contractRenameTitleSchema.parse(await request.json())

    const contractQueryService = getContractQueryService()
    const contractView = await contractQueryService.renameTitle({
    ...
    if (error instanceof ZodError) {
      return NextResponse.json(errorResponse('VALIDATION_ERROR', 'Invalid rename title payload'), { status: 400 })
    }

    const status = isAppError(error) ? error.statusCode : 500
    const code = isAppError(error) ? error.code : 'INTERNAL_ERROR'
    const message = isAppError(error) ? error.message : 'Failed to rename contract'
