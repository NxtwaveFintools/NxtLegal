...
const requestSchema = z.object({
  ...
  isFinalExecuted: z.boolean().optional().default(false),
})
...
const POSTHandler = withAuth(async (request: NextRequest, { session, params }) => {
  ...
  try {
    ...
    const document = await contractUploadService.finalizeReplacePrimaryDocument({
      ...
      isFinalExecuted: parsedPayload.data.isFinalExecuted,
    })
    ...
    logger.warn('Main document replacement finalize failed', {
      error: String(error),
      errorCode: isAppError(error) ? error.code : 'INTERNAL_ERROR',
    })
    const status = isAppError(error) ? error.statusCode : 500
    const code = isAppError(error) ? error.code : 'INTERNAL_ERROR'
    const message = isAppError(error) ? error.message : 'Failed to finalize main document replacement'
