...
const POSTHandler = withAuth(async (request: NextRequest, { session, params }) => {
  ...
  try {
    ...
    const plan = await contractUploadService.initiateReplacePrimaryDocument({
    ...
    logger.warn('Main document replacement init failed', {
      error: String(error),
      errorCode: isAppError(error) ? error.code : 'INTERNAL_ERROR',
    })
    const status = isAppError(error) ? error.statusCode : 500
    const code = isAppError(error) ? error.code : 'INTERNAL_ERROR'
    const message = isAppError(error) ? error.message : 'Failed to initialize main document replacement'
