-- Hard cutover: remove legacy workflow statuses/transitions and enforce canonical state model.
...
SET is_active = FALSE,
    updated_at = NOW()
WHERE trigger_action IN ('system.route_to_legal', 'legal.approve', 'legal.query')
   OR from_status IN ('LEGAL_PENDING', 'LEGAL_QUERY', 'FINAL_APPROVED', 'IN_SIGNATURE', 'HOD_APPROVED')
   OR to_status IN ('LEGAL_PENDING', 'LEGAL_QUERY', 'FINAL_APPROVED', 'IN_SIGNATURE', 'HOD_APPROVED');

DELETE FROM public.contract_transition_graph
